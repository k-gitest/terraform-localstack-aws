...
```mermaid
graph TD
    A[アクセスリクエスト] --> B{リクエストに適用される<br/>すべてのポリシーを収集};
    
    B --> C{Step 1: 適用されるポリシーの<br/>**いずれか**に**明示的な拒否**がありますか？};
    
    C -- Yes (Explicit Deny) --> D[❌ 最終決定: アクセス拒否];
    
    C -- No (Denyなし) --> E{Step 2: 適用されるポリシーの<br/>**いずれか**に**明示的な許可**がありますか？};
    
    E -- Yes (Explicit Allow) --> F[✅ 最終決定: アクセス許可];
    
    E -- No (Allowなし) --> G[❌ 最終決定: 暗黙的な拒否];
```
